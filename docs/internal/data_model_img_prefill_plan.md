...
**Audit 2026-08-03 (full repo re-verification against this plan).** T0, T1, T3, T6a, T6b, T7, T8, T9, T10, T11, T12 confirmed shipped by inspection. Four corrections to the claims above:

1. **T13 single-item image slots are NOT shipped.** There is no `image_role` field on `ListingImage` / `CollectionItemImage` — grep returns zero hits. "Featured"/"Back" exist only as client-side badges keyed off array index (`listing_create.html:724-734`), so reordering the drag-and-drop grid relabels them and nothing persists. Acceptance #13 is cosmetically met, structurally not. Schema work still owed; rides 10.15 with the lot slots.
2. **T2 does not surface in listing detail.** `templates/listings/listing_detail.html` renders neither `item_kind` nor `addons_attached` (collection detail does, at `collection_item_detail.html:53,60`). Also note the widget shipped as `NullBooleanSelect`, not the checkbox this plan specifies — deliberate and correct for a 3-state field, recording the deviation.
...
| 13 | (10.8/10.15) Create flow is two pages (config → details) ✅; single-item forms show labeled Featured/Back slots (Mona Lisa placeholder kept) ⬜ **not met — `image_role` does not exist; the labels are index-derived badges only**; lots capture 2–10 items with Front/Back per item + one group Featured image ⬜; >10 routes to box-lot guidance ⬜. |
