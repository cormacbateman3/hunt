...
**Execution status (updated 2026-07-22, branch `feature/alpha-p3-t0-t14`):** T0–T5, T6a, T7, **T8**, T10, T11 Phase A, and T12 are **shipped and verified** (research applied to ref_data, migrations + backfill run, seeders drift-free and idempotent, API year-gating live, forms/admin wired). T8 live 17-image run: every diagnosed failure case fixed (Turkey Tag [high], MD Deer Tag [high], Muzzle Loader matched via alias + second pass, county-number ⇒ PA prior fired, serial suffix preserved, addon kind gating). Follow-ups landed 2026-07-22: PA **Deer Tag** + **Small Game Tag** rows added (run-revealed vocab gap, artifact evidence from the 1979 test item); `MAX_IMAGE_EDGE` 1120 → **1568** (readability over cost, per Cormac); prompts/schema/aliases/knobs **externalized to `prefill/config/`** (`system_prompt.md`, `extraction_tool.json`, `second_pass_prompt.md`, `concept_aliases.json`, `config.json` — originally written to `sandbox/prefill_config/`, moved when 10.5 productionized the package) with a `PROMPT_VERSION` hash — nothing prompt-like is hardcoded in Python anymore, and `PrefillJob.prompt_version` consumes the hash in 10.5. 2026-07-28 frontend fix: the 10.8 chip-checkbox Color(s) field emitted `sel: '#'` into the KBPrefill config (Django 5 checkbox groups have no id_for_label), which crashed init and silently disabled autofill on all three forms — colors rewired as a checklist group, `el()` hardened against invalid selectors, 3 page-render regression tests added. Same-day second fix: CSRF_COOKIE_HTTPONLY blocked the JS cookie read → empty X-CSRFToken → HTML 403 → "Unexpected token '<'"; prefill.js now reads the hidden csrfmiddlewaretoken input, fetches parse defensively, and create_job converts pipeline crashes to JSON failures. Live end-to-end smoke through Django: complete, PA/1969 high-tier, $0.0045/img.

**Prefill polish deliberately deferred** (model is good-enough; revisit after core workflows): R6 gold labels + per-tier precision scoring (notebook cell is ready), serial agreement-gating, threshold tuning against the gold set, prompt iteration on the remaining single-image misses. Remaining elsewhere: ~~T6b/T9~~ **shipped with 10.8 (2026-07-23)** — item_kind conditional sections + kind-aware completeness live on all three forms; **T13 two-page create flow shipped with 10.8**; T13 lot images ride 10.15; T14 rides 10.18. Postgres sanity check pending a staging DB.

**Audit 2026-08-03 (full repo re-verification against this plan).** T0, T1, T3, T6a, T6b, T7, T8, T9, T10, T11, T12 confirmed shipped by inspection. Four corrections to the claims above — **and two of those corrections themselves went stale within days; see the 2026-08-06 note under each**:

1. ~~**T13 single-item image slots are NOT shipped.**~~ **Superseded 2026-08-06:** the schema landed with the Pass 5 sell-flow work this audit predated by hours-to-days — `image_role` on both `ListingImage` (migration `0019`) and `CollectionItemImage` (`0010`), each with a one-front-one-back `UniqueConstraint`. What genuinely remained was the UI half, and Pass 8b built it: named Front/Back slots that **write the role with the file** (`static/js/item-form.js`), so reordering the details can never relabel the named photographs. Acceptance #13's single-item half is now met structurally, not just cosmetically. The lot slots still ride 10.15.
2. ~~**T2 does not surface in listing detail.**~~ **Superseded 2026-08-06:** `listing_detail.html` prints both — *Form* (`get_item_kind_display`) and *Add-ons: Still attached / Detached* — in "The object" spec group; it has since the Pass 1/5 listing-detail rebuild. The widget note stands: `NullBooleanSelect`, not a checkbox, deliberate for a 3-state field — and Pass 8b gave its three states honest words (*Not sure / Yes, uncut / No — cut or detached*, `apps/core/widgets.py`).
3. **T5 is now genuinely complete (fixed 2026-08-03).** Three acquisition-method variants had survived the 07-10 sweep: KS "Deer Archery Tag (OTC)", KS "Antlerless Whitetail Tag (OTC)", KY "Elk Draw Permit". Renamed in `addons_permits.csv` with matching `RENAMES` entries; re-seed reported `renamed=3`, a second run was a clean no-op, and no orphaned rows remain. Acceptance #5 now holds.
4. **T4's remaining blanks are correct, and acceptance #4 was mis-worded** — see the amended criterion. The four PA integral-tag rows (Turkey Tag, Big Game Tag, Deer Tag, Small Game Tag) carry `approx_last_year` but no `approx_first_year` because R2 established the first year is genuinely unknown. Verified on the live API (`/api/license-types/?state=PA&year=1969`): filling those floors would not improve disambiguation anyway, because the competing "Special Spring Turkey Permit" (1968–) is *also* in range for 1969. The upper bound is what does the work; the 1969 Turkey Tag case is won by vocabulary + exact-name match, not era gating.

**Pass 8b additions (2026-08-06):** the `issue_class` taxonomy category joined T5 (entry there), and `CollectionItem.disposition` landed alongside — a collections-app fact recorded in `plan_design.md`, mentioned here only because the ledger/export tasks (T14, 10.18) will want it. Both forms' taxonomy field lists now come from one constant (`FORM_TAXONOMY_FIELDS` in `apps/core/constants.py`), which is where a new category gets its form label.

No automated test covers T7 year gating, T11 drift/`--overwrite`, or the `RENAMES` M2M repointing — all three verified by hand only.
...
- **New category `issue_class` (added 2026-08-06, Pass 8b — the design plan's inbox holds the reasoning).** *Special Issue / Limited Edition / Commemorative* + Other, universal rows seeded through the pipeline like materials (`ISSUE_CLASSES` in `clean_reference_data.py`; regen lossless, seeder `created=4 drift=0 total=517`). A property of the **issue**, deliberately not an `addon_type` value — a Special Issue turkey tag is both, and one category holding both would force a collector to choose between two true things. Inherits the form field, the Other→suggestion flow, the API grouping and the browse rail because everything reads `FORM_LICENSE_TYPE_CATEGORIES`. The prefill extraction schema does not return it — enrichment is human, via admin or suggestions, until artifact evidence says otherwise.
...
1. **Year ranges** (T4 / R3): ✅ researched 2026-07-10, ✅ **applied to the CSV** (verified 2026-08-03). 21 of 205 addon rows now carry `approx_first_year`; 6 carry `approx_last_year`. By state: PA 12/16, FEDERAL 2/2, MD 3/10, OH 4/7. Remaining blanks (MD Archery/Muzzleloader/Sika/Bonus/Furbearer; OH Archery/Muzzleloader/Antlerless; the 4 PA integral tags) stay empty low-confidence — they gate suggestions only; fill opportunistically (MD DNR guide back-issues / ODNR digests per the research doc). **Do not guess a floor to satisfy a checkbox** — a written year is treated as authoritative by the resolver, a blank one is not.
2. **Historical addon/license rows** (T5 / R2, R4): ✅ researched, ✅ **applied to the CSV** (verified 2026-08-03). All present: PA ×7 (Turkey Tag, Big Game Tag, Antlerless Deer License, Muzzleloader License, Archery License, plus the run-revealed Deer Tag + Small Game Tag) and MD ×2 (Deer Tag, Turkey Tag). Of the optional OH pair, Deer Tag (1943) was added; Fall Turkey Permit was not.
...
| 4 | `first_year`/`last_year` populated for all PA addon rows and federal stamps **where the year is determinable**; documented low-confidence blanks are an acceptable outcome, not a gap (amended 2026-08-03 — the original "all rows" wording invited inventing data the research explicitly could not establish). API `year` param orders/flags accordingly. ✅ |
| 5 | No draw/OTC variants remain in the taxonomy; historical minimum set seeded with facets. ✅ (3 stragglers swept 2026-08-03) |
...
| 13 | (10.8/10.15) Create flow is two pages (config → details) ✅; single-item forms show labeled Featured/Back slots ✅ **met 2026-08-06 — `image_role` persisted on both image models with one-per-slot constraints, and the Pass 8b slot UI writes the role with the file** (the Mona Lisa placeholder gave way to the design's hatch fill); lots capture 2–10 items with Front/Back per item + one group Featured image ⬜; >10 routes to box-lot guidance ⬜. |
