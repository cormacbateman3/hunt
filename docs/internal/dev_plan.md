...
Data/DB: new Follow (follower, followed, created_at). Reviews/favorites need no new schema.
Acceptance: listing detail always links to the seller and shows trust summary; completed orders/trades unlock one review each; favorite counts are visible; profiles have an Active Listings tab; users can follow and filter by Following.
