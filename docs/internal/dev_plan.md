...
Status 2026-07-31 audit+fix pass: ran a 13-agent adversarial audit of every 10.8 scope item against the code AND the rendered pages; 19 findings survived verification, backend/correctness batch now fixed (suite 33→43 green). Year bounds were server-only — now mirrored client-side (min/max on the input, state-driven via geo API, help text healing the dangling aria-describedby) with a 1850 absolute floor and a data-derived 1934 Federal floor (was NULL, letting nonsense years save); hidden base-dimension picks no longer save/validate when item_kind=addon (was corrupting the taxonomy M2M); three lossy copy paths fixed (re-list from collection, auto-created collection item, auto-relist — the last rewritten to clone the row so no future field is silently dropped); negative-price exploit closed with MinValueValidators on all prices + Bid.amount; bid-minimum logic unified into minimum_bid_for() so detail page, bid form, and place_bid agree, and the first-bid rejection names the starting price not the increment; .errorlist now styled (--color-error defined) so field errors are visible; taxonomy group opens only on its own errors; shipping group opens on any shipping-field error; "License or add-on?" label disambiguated; prefill badge-clear re-runs the kind toggle. Follow-up commit ported the create page's structure to the edit form (top error summary with anchor links + novalidate; Item taxonomy / Listing options / Shipping now auto-collapsed <details> with scoped error-open conditions) and gave the collection form the same collapsible taxonomy group — all three forms now share layout and validation behavior (pages render 200, tag-balanced, 43 tests green). Still deferred: the 5 auditors (validation-loudness, colors, defaults/banner, JS, parity) + the 10.9 design panel that died at a session limit — not re-run (the 19 confirmed findings covered the substantive issues; those five dimensions overlapped auditors that did complete).
Status 2026-07-28 fix: image-prefill autofill never started on the overhauled forms — Django 5 renders an empty id_for_label for checkbox groups, so the chip-based Color(s) field made the KBPrefill config emit sel: '#', and that invalid selector crashed init before the upload listener attached. Colors is now wired as a checklist group on all three forms (listing create/edit, collection item), prefill.js ignores invalid selectors instead of dying, and 3 page-render regression tests pin the config. Second root cause found on retest: CSRF_COOKIE_HTTPONLY=True makes the csrftoken cookie unreadable from JS, so the prefill POST carried an empty X-CSRFToken and got Django's HTML 403 back ("Unexpected token '<'"). prefill.js now reads the form's hidden csrfmiddlewaretoken input (documented Django pattern — the cookie still rides along for validation), all prefill fetches parse defensively (HTML error pages surface as "server error (HTTP nnn)"), and create_job catches pipeline crashes into a JSON failed status. Verified end-to-end with a live API call through the Django stack (job complete, PA/1969 high-tier, $0.0045). Suite at 33 green.
Status 2026-07-23: DONE — two-page create flow (config page: marketplace + source → tailored details page with a "Selling in … (change)" chip); loud validation (error summary with anchor links at the top; found+fixed the actual silent-failure cause: new model fields defaulting to form-required); dynamic year bounds (state min, current−25) on both forms; sections regrouped with auto-collapsed groups (Item taxonomy — opens itself on errors, Listing options, Shipping) and Issuing State moved below Images; multiple Add-on Types + Color(s) as chip checkboxes (state-aware refresh rebuilds checklists; "Other" detection handles chip groups); Era (Circa) auto-fills from year; price inputs step $1 with $-hints; trade removed from listing-type choices; bid_increment (seller-set, enforced in place_bid); listing templates via profile-settings Listing Defaults (UserProfile.listing_defaults) applied to new listings; improved interim condition grades + extra colors; image zone: same-file re-drop dedupe, from-collection copy skipped when the seller uploads photos (the "reorder duplicates" cause), styled in-app confirm modal, Featured + Back slot badges (T13 single-item); "Is a value missing?" reworked (guidance copy, no Current Value for New Value, free-text for Other); dev-facing PA banner replaced; data-model plan T6b (item_kind conditional sections incl. kind-aware labels) + T9 (kind-aware completeness) shipped; prefill applies every matched add-on via the checklist. Out-of-state unit was already selectable via T0 reference data. 5 new tests; suite at 28 green.
Scope (from todo.txt)
...
DIRECTION CHANGE 2026-07-31: the CART is dropped — a multi-item cart doesn't fit a marketplace where listings come from different sellers with different shipping. The real problem is that the current one-click Buy Now is "just not right" (jumps straight from a single click to Stripe with no review, and locks the listing on click). So 10.9 = fix the buy-now checkout flow + keep Offers; no Cart/CartItem models.
Status 2026-07-31 (slice 1 of 10.9): buy-now review/confirm page shipped. Clicking "Buy now" now opens a per-listing review page (item, shipping estimate, platform fee, total) that creates NO Order and does NOT lock the listing — the listing only locks (status→pending, Order created) when the buyer clicks "Proceed to secure payment", which is the existing buy_now_checkout_start → Stripe path (delist-to-sold still happens only on the paid webhook; 30-min stale release unchanged). New: listings.buy_now_review view + URL + templates/listings/buy_now_review.html; shipping.estimate_listing_shipping() (best-effort quote, no Order/Shipment persisted; seller-pays shows "Free"); detail-page "Buy now" is now a link to the review page. 6 new tests (suite 49 green). Still to do in 10.9: Offers (Make Offer + seller counter, "Allow Offers" per listing, accepted offer → same checkout), and reviewing the auction win-to-pay path for consistency.
Status 2026-08-03 (slice 2 of 10.9): OFFERS SHIPPED. New `apps.offers` app (matching how apps/bids owns auction bidding and apps/trades owns trade negotiation) with `Offer` (listing, from_user, to_user, amount, status pending/accepted/declined/countered/withdrawn/expired, counter_to self-FK, expires_at, accepted_at) and `Listing.allow_offers` (migration listings/0018). Buyers originate below list price; sellers may only counter (`counter_to` required for a seller-authored offer) — the spec's anti-spam rule is enforced in services.create_offer, not just the UI. Accepting declines all other pending offers on the listing.
Price handoff: `Order.item_amount` is the single point everything downstream derives from (platform fee, shipping recalc, Stripe unit_amount), so buy_now_review + buy_now_checkout_start now price from the accepted offer instead of `listing.buy_now_price`. Verified the platform fee is computed off the offer, not list price — pricing off list would have overcharged the buyer.
Reservation design (worth remembering): accepting does NOT set `listing.status='pending'`. There is no Order at acceptance time, so the 30-minute stale-order sweeper could never release it and the listing would freeze forever. Instead an accepted offer reserves the listing via a check (`reserving_offer()`), and the reservation lapses on its own after a 48h payment window (`Offer.reservation_lapsed`, a property so it takes effect immediately rather than waiting on cron). `manage.py expire_offers` sweeps both stale pending offers and lapsed acceptances.
Also: 5 new notification types (notifications/0006), offer detail/form/my-offers templates, "Make an offer" + seller's pending-offer list on listing detail, "Allow offers" checkbox on the create + edit forms. 30 new tests (suite 49 → 79 green), plus a scripted end-to-end HTTP walkthrough of buyer-offer → seller-counter → buyer-accept → checkout confirming the Order is written at the agreed price.
Status 2026-08-03 (slice 3 of 10.9 — COMPLETE): auction win-to-pay brought in line with buy-now. The audit found the auction path diverged on six counts; five are fixed, one is deliberately deferred.
**Money bug found and fixed:** `close_auctions` created every auction Order with `platform_fee_amount = 0` and `total = item_amount`. Because `apply_shipping_to_order` rebuilds the total from item + platform_fee, the zero survived all the way to the Stripe charge — **the platform fee was silently lost on every auction sale since the Order model landed**. The same hole existed in `payments/views._get_or_create_order_from_transaction`. Both now call a new `orders.services.build_order_amounts()`, which is the single source of truth for order pricing so the two paths cannot drift again.
**Delist only on payment:** `close_auctions` set the listing to `'sold'` at close, before a cent was paid. It now sets `'pending'` (won, awaiting payment) and the paid webhook completes the sale — the webhook's delist condition was `order_type == 'buy_now'` only and is now widened to both. This is the spec's "the listing is only delisted on successful payment" applied to the Auction House.
**Review before Stripe:** new `listings:auction_win_review` (`/listings/<pk>/won/review/`) mirrors `buy_now_review` — winning bid, platform fee, shipping estimate, total. The close notification previously pasted a raw Stripe checkout URL into the message body, so a winner could land on the payment page having reviewed nothing; it now links to the review page. Note the Order is still created at close, unlike buy-now: winning an auction *is* the commitment, so there is nothing to defer.
**Release for unpaid wins:** required by the delist change (the listing now sits in `'pending'`, and the buy-now sweeper is scoped to `order_type='buy_now'`). New `orders.services.release_unpaid_auction_wins()` + `manage.py release_unpaid_auction_wins` cancels the order, fails the payment, moves the listing to `'expired'` and notifies the seller. Runs on the same 24h grace as the existing non-payment strike.
**Detail page:** a closed auction still rendered a live "Place bid" form (no status guard) and the winner had no route to payment at all. Now: winner sees "You won — Review & pay", everyone else sees "This auction has ended", and the bid form is gated on the auction actually being open. `my_orders` gained a Review & pay CTA for both order types.
Also: `auction_won` was a fully-wired notification type (subject + email template) that was never emitted — the winner got a generic `order_created`. Now emitted, with a dedupe window.
**Deferred deliberately:** second-chance offers to the next-highest bidder when a winner defaults. Nothing exists today, it is a product-policy decision rather than a consistency fix, and it is not in the 10.9 scope list. Worth a 10.13 or Beta slot.
apps/bids/tests.py went from nonexistent to 12 tests (the auction close→order→payment path previously had zero coverage). Suite 84 → 92 green, plus an HTTP walkthrough of close → winner review → CTA states.
Status 2026-08-03 (self-dealing hardening): audited every path by which a user could buy or bid on their own listing. The view/form/service guards were all present and correct (BidForm.clean + place_bid, buy_now_review + buy_now_checkout_start, offers make_offer + create_offer), but two of the three Order-creation paths had no check of their own: `close_auctions` (a seller's bid could win and mint a buyer==seller Order) and `payments._get_or_create_order_from_transaction` (legacy Transaction rows bypass the checkout guards entirely). Added, in layers: a DB `CheckConstraint` `order_buyer_is_not_seller` and `offer_parties_are_distinct` (portable SQL, verified against the generated DDL); a `Bid.save()`/`clean()` guard, since the seller lives on Listing and a check constraint cannot reach across an FK, so shell/admin/import paths were unprotected; `close_auctions` now excludes the seller's own bids from winner selection so a pre-guard row cannot win; and the legacy transaction path raises rather than surfacing an IntegrityError. 13 tests in the new apps/orders/tests.py cover each layer independently, so deleting any single guard fails a test instead of quietly opening a hole. Suite 92 → 105 green.
Scope (from todo.txt)
...
reversing decision on adding cart - it doesn't make sense when you are using multiple sellers (different shipping). it is way more the current one click buy now that is just not right.
