...
DIRECTION CHANGE 2026-07-31 (Cormac): the CART is dropped — a multi-item cart doesn't fit a marketplace where listings come from different sellers with different shipping. The real problem is that the current one-click Buy Now is "just not right" (jumps straight from a single click to Stripe with no review, and locks the listing on click). So 10.9 = fix the buy-now checkout flow + keep Offers; no Cart/CartItem models.
